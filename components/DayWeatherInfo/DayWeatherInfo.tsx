...
const DayWeatherInfo = ({
  ...
  return (
    <ul className={`grid grid-cols-2 justify-items-center sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2 lg:gap-6 w-full`}>
      {weatherInfo.map((timeInfo) => {
        return (
          <div
            key={timeInfo.dt}
            className={`w-fit h-fit px-4 py-2 md:px-8 md:py-4 flex flex-col bg-white rounded-[15px] outline outline-1`}>
